         ...
         {recentSales?.map((sale) => (
            <div key={sale?.sales?.salesId} className="flex items-center">
               <Avatar className="h-9 w-9">
                  <AvatarFallback>{getFirstLetters(sale.customers.customerName, 2)}</AvatarFallback>
               </Avatar>
               <div className="ml-4 space-y-1">
                  <p className="text-sm font-medium leading-none">{sale.customers.customerName}</p>
                  <p className="text-sm text-muted-foreground">{sale.customers.customerContact}</p>
               </div>
